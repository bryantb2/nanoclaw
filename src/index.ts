 ...
 */
/**
 * Synthetic IDs generated by injectMessage() (from IPC routing) use the
 * `ipc-{ISO-timestamp}-{rand}` format. These are NOT valid Slack thread
 * timestamps and will be rejected by Slack's chat.postMessage API with
 * "invalid_thread_ts". Filter them out of thread anchor selection.
 */
export function isValidThreadTs(id: string | undefined): boolean {
  return !!id && !id.startsWith('ipc-');
}
...
export function selectThreadMessage(
  ...
  if (isMain) {
    // Walk backwards to find the latest message with a real Slack ts
    for (let i = messages.length - 1; i >= 0; i--) {
      if (isValidThreadTs(messages[i].id)) return messages[i];
    }
    return undefined;
  }
...
async function processGroupMessages(chatJid: string): Promise<boolean> {
  ...
  // selectThreadMessage already filters synthetic ipc- ids, but double-check
  // defensively in case a future caller bypasses it.
  const threadTs = isValidThreadTs(threadMsg?.id) ? threadMsg?.id : undefined;
    ...
    try {
      ...
      if (messages.length > 0) {
        ...
        for (const [chatJid, groupMessages] of messagesByGroup) {
          ...
          if (pipedThreadMsg && isValidThreadTs(pipedThreadMsg.id)) {
            latestThreadTs[chatJid] = pipedThreadMsg.id;
          }
